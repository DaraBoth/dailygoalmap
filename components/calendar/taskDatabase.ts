...
export async function saveTaskToSupabase(task: Task, goalId: string): Promise<void> {
  ...
  const publicBase = process.env.NEXT_PUBLIC_PUBLIC_URL || (typeof window !== 'undefined' ? window.location.origin : undefined);
...
export async function updateTaskCompletion(taskId: string, completed: boolean): Promise<void> {
  ...
  const publicBase2 = process.env.NEXT_PUBLIC_PUBLIC_URL || (typeof window !== 'undefined' ? window.location.origin : undefined);
